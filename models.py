...
class AdaptiveSequenceProcessor(nn.Module):
    ...
    def forward(self, features):
        ...
        if seq_len <= self.short_threshold:
            ...
            return self._process_long_sequence(features)
    
    def _process_short_sequence(self, features):
        attended_features, attention_weights = self.short_attention(features, features, features)
    ...
    def _process_medium_sequence(self, features):
        ...
        local_span = min(self.min_span * 2, features.shape[0])
        local_features = features[-local_span:]
        local_attended, _ = self.local_attention(local_features, local_features, local_features)
        
        # Global attention
        if features.shape[0] > 16:
            step = max(1, features.shape[0] // 16)
            global_features = features[::step]
            global_attended, _ = self.global_attention(global_features, global_features, global_features)
            ...
            if global_upsampled.shape[0] > features.shape[0]:
                ...
                padding = features.shape[0] - global_upsampled.shape[0]
                global_upsampled = torch.cat([global_upsampled, global_upsampled[-padding:]], dim=0)
        else:
            global_upsampled = local_attended
        
        # Combine features
        local_extended = torch.cat([features[:-local_span], local_attended], dim=0)
    ...
    def _process_long_sequence(self, features):
        seq_len, batch_size, dim = features.shape
        context = torch.mean(features, dim=0)
        span_weights = self.span_predictor(context)
        ...
        combined = torch.cat([
            local_stack * span_weights[:, 1:2],
            global_stack * span_weights[:, 2:3]
        ], dim=-1)
...
class HierarchicalContextEncoder(nn.Module):
    ...
    def forward(self, features, sequence_length_ratio=1.0):
        ...
        if seq_len <= 16:
            ...
            downsample_factor = min(4, max(2, seq_len // 16))
            coarse_features = features.permute(1, 2, 0)
            coarse_features = F.avg_pool1d(coarse_features, kernel_size=downsample_factor, 
                                         stride=downsample_factor, padding=0)
            coarse_features = coarse_features.permute(2, 0, 1)
            
            coarse_encoded = self.coarse_encoder(coarse_features)
            
            coarse_upsampled = coarse_encoded.permute(1, 2, 0)
            coarse_upsampled = F.interpolate(coarse_upsampled, size=seq_len, 
                                           mode='linear', align_corners=False)
            coarse_upsampled = coarse_upsampled.permute(2, 0, 1)
        ...
        if seq_len > 16:
            consistency_features = selected_features.permute(1, 2, 0)
            consistency_features = self.temporal_consistency(consistency_features)
            consistency_features = consistency_features.permute(2, 0, 1)
...
class DETRInspiredModule(nn.Module):
    """DETR-inspired module for better object detection and localization."""
    def __init__(self, embedding_dim, num_heads, num_queries, dropout=0.1):
        super(DETRInspiredModule, self).__init__()
        self.num_queries = num_queries
        self.embedding_dim = embedding_dim
        
        # Learnable object queries (similar to DETR)
        ...
        # Hungarian matching inspired feature enhancement
        ...
        # Positional guidance for better localization
        self.position_guide = nn.Sequential(
            nn.Linear(embedding_dim, embedding_dim // 2),
            nn.GELU(),
            nn.Linear(embedding_dim // 2, 2),  # For start/end prediction
            nn.Sigmoid()
        )
    ...
    def forward(self, encoded_features):
        """
        Args:
            encoded_features: [seq_len, batch_size, embedding_dim]
        Returns:
            enhanced_queries: [num_queries, batch_size, embedding_dim]
            attention_weights: Attention weights for interpretability
        """
        seq_len, batch_size, embedding_dim = encoded_features.shape
        
        # Expand object queries for batch processing
        queries = self.object_queries.unsqueeze(1).expand(-1, batch_size, -1)
        
        # Query self-attention
        queries_attended, query_attn = self.query_self_attention(queries, queries, queries)
        queries = self.layer_norm1(queries + queries_attended)
        
        # Cross-attention between queries and encoded features
        cross_attended, cross_attn = self.cross_attention(
            query=queries,
            key=encoded_features,
            value=encoded_features
        )
        queries = self.layer_norm2(queries + cross_attended)
        
        # FFN processing
        queries_ffn = self.query_ffn(queries)
        queries = self.layer_norm3(queries + queries_ffn)
        
        # Feature enhancement
        enhanced_queries = self.feature_enhancement(queries)
        
        # Position guidance and confidence scoring
        position_hints = self.position_guide(enhanced_queries)
        confidence_scores = self.confidence_scorer(enhanced_queries)
        
        # Combine position hints and confidence into enhanced features
        enhanced_queries = enhanced_queries * confidence_scores
        
        return enhanced_queries, cross_attn, position_hints, confidence_scores
...
class MYNET(torch.nn.Module):
    def __init__(self, opt):
        ...
        # DETR-inspired module
        self.detr_module = DETRInspiredModule(
            embedding_dim=n_embedding_dim,
            num_heads=n_enc_head,
            num_queries=len(self.anchors),
            dropout=dropout
        )
        ...
        # Enhanced heads with DETR features
        self.classifier = nn.Sequential(
            nn.Linear(n_embedding_dim * 2, n_embedding_dim),  # Enhanced with DETR features
            nn.GELU(), 
            nn.LayerNorm(n_embedding_dim),
            nn.Dropout(dropout),
            nn.Linear(n_embedding_dim, n_class)
        ...
        # Auxiliary regression head for position hints
        self.position_regressor = nn.Sequential(
            nn.Linear(2, n_embedding_dim // 4),
            nn.GELU(),
            nn.Linear(n_embedding_dim // 4, 2)
        )
    ...
    def forward(self, inputs):
        ...
        if seq_len <= self.short_threshold:
            processed_features, _ = self.adaptive_processor(pe_x)
            encoded_x = processed_features
            for i in range(min(3, len(self.encoder_layers))):
                encoded_x = self.encoder_layers[i](encoded_x)
            ...
            hierarchical_features = self.hierarchical_encoder(pe_x, sequence_length_ratio)
            processed_features, _ = self.adaptive_processor(hierarchical_features)
        ...
        # DETR-inspired processing
        detr_queries, cross_attn, position_hints, confidence_scores = self.detr_module(encoded_x)
        
        # Combine decoder tokens with DETR queries
        decoder_token = self.decoder_token.expand(-1, encoded_x.shape[1], -1)
        enhanced_decoder_features = torch.cat([decoder_token, detr_queries], dim=-1)
        
        enhanced_decoder_features = enhanced_decoder_features.permute([1, 0, 2])
        
        # Enhanced predictions
        anc_cls = self.classifier(enhanced_decoder_features)
        anc_reg = self.regressor(enhanced_decoder_features)
        
        # Auxiliary position regression
        position_hints = position_hints.permute([1, 0, 2])
        aux_reg = self.position_regressor(position_hints)
        
        # Combine main regression with auxiliary hints
        anc_reg = anc_reg + aux_reg * 0.1  # Small weight for auxiliary signal
